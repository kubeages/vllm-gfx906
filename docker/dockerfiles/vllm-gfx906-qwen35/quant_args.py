...
class QuantizationArgs(BaseModel, use_enum_values=True):
    ...
    # Added for forward-compatibility with compressed-tensors >= 0.14.x
    # These fields exist in newer quantization configs (e.g. cyankiwi Qwen3.5 AWQ)
    # but are not used at inference time — we accept and ignore them.
    zp_dtype: Optional[Any] = Field(
        default=None,
        description="Zero-point dtype hint from compressed-tensors >= 0.14. Ignored at inference.",
    )
    scale_dtype: Optional[Any] = Field(
        default=None,
        description="Scale dtype hint from compressed-tensors >= 0.14. Ignored at inference.",
    )
